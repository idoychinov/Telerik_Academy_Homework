...
class Catalan
{
    static void Main()
    {
        uint N;
        decimal cat = 1, temp;
        MyIOClass ConsoleIO = new MyIOClass(); //creating new object from the custom class in order to use it for input of the variables.

        while (true)
        {
            ConsoleIO.Input(out N, "Enter the number N: ");
            if (N > 0)
            {
                break;
            }
