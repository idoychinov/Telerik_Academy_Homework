namespace Cars.Tests.JustMock.Mocks
{
    ...
    public class MoqCarsRepository : CarRepositoryMock, ICarsRepositoryMock
    {
        protected override void ArrangeCarsRepositoryMock()
        {
            ...
            mockedCarsRepository.Setup(r => r.Search("")).Returns(this.FakeCarCollection);
            mockedCarsRepository.Setup(r => r.Search("Astra")).Returns(this.FakeCarCollection.Where(c => c.Model == "Astra").ToList());
            mockedCarsRepository.Setup(r => r.GetById(It.IsAny<int>())).Returns(this.FakeCarCollection.First());
            //fixed get data mock
            mockedCarsRepository.Setup(r => r.GetById(It.IsAny<int>())).Returns(this.FakeCarCollection.First());
            mockedCarsRepository.Setup(r => r.GetById(-1)).Returns<Car>(null);
            //Added sorted Mocks
